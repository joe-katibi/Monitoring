...
@section('title', 'livecall')

@section('content_header')
@stop

@section('content')
<form action="{{ route('livecalls') }}" method="POST" name="listForm">
    {{csrf_field()}}
    <div class="card card-success">
       <div class="card-header">
         <input readonly class="form-control" style="color: green" name="category" value="{{ $tittle->category_name }}">
         <input hidden readonly type="text" class="form-control" name="quality_analysts"
      ...
      <div class="card-body">
          <div class="row">
              ...
              <div class="col-sm-6">
                   ...
                   <option name="strength_summary[{{$item['id']}}]" value="{{$item['id']}}">{{$item['summary_name']}}</option>
                    @endforeach
                   </select>
